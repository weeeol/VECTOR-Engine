...
3. **The Loop**: `Application::Run()` manages the core game loop:
   ...
   - **Rendering**: Calls `Render()`. The engine manages a centralized render pipeline (`Renderer::ExecuteRenderPipeline()`). The active `Scene` iterates over entities with `RenderComponent`, `MeshComponent`, and `TransformComponent` and submits their data to the `Renderer`. The `Renderer` is an abstract interface implemented by both `VulkanRenderer` and `DirectX12Renderer` (featuring Shadow Mapping, SSAO, Bloom, TAA, and Post-Processing passes). The Renderer also handles drawing the 2D orthographic UI on top of the 3D scene using Dear ImGui and SDL abstractions.
