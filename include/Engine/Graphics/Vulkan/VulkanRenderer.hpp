...
public:
  ...
  virtual bool Initialize(const std::string &title, int width,
                          int height) override;
  ...
  virtual std::string GetRendererInfo() const override {
    if (!VulkanContext::Get() || !VulkanContext::Get()->GetPhysicalDevice()) return "Vulkan";
    VkPhysicalDeviceProperties props;
    vkGetPhysicalDeviceProperties(VulkanContext::Get()->GetPhysicalDevice(), &props);
    return std::string("Vulkan - ") + props.deviceName;
  }
...
private:
  ...
  struct MaterialTexturesHash {
    std::size_t operator()(const MaterialTextures &t) const {
      return std::hash<const void *>()(t.albedo) ^
             (std::hash<const void *>()(t.normal) << 1) ^
             (std::hash<const void *>()(t.mr) << 2) ^
             (std::hash<const void *>()(t.ao) << 3);
    }
  };

  std::unordered_map<MaterialTextures, VkDescriptorSet, MaterialTexturesHash>
