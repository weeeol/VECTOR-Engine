...
void main() {
    ...
    vec4 worldPos = pc.model * totalPosition;
    vec4 viewPos = pfd.view * worldPos;
    
    gl_Position = pfd.projection * viewPos;

    mat3 normalMatrix = transpose(inverse(mat3(pfd.view * pc.model)));
    ViewNormal = normalize(normalMatrix * totalNormal);
    ViewPos = viewPos.xyz;
}
