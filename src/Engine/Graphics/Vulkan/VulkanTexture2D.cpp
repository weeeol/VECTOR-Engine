...
namespace VECTOR {
    ...
    VulkanTexture2D::VulkanTexture2D(uint32_t width, uint32_t height, TextureFormat format)
        ...
        if (isDepth) {
            ...
            switch (format) {
                case TextureFormat::RGBA16F: imageInfo.format = VK_FORMAT_R16G16B16A16_SFLOAT; break;
                case TextureFormat::RGBA32F: imageInfo.format = VK_FORMAT_R32G32B32A32_SFLOAT; break;
                case TextureFormat::RG16F: imageInfo.format = VK_FORMAT_R16G16_SFLOAT; break;
                case TextureFormat::RG32F: imageInfo.format = VK_FORMAT_R32G32_SFLOAT; break;
                default: imageInfo.format = VK_FORMAT_R16G16B16A16_SFLOAT; break;
            }
        }
        imageInfo.tiling = VK_IMAGE_TILING_OPTIMAL;
        imageInfo.initialLayout = VK_IMAGE_LAYOUT_UNDEFINED;
        imageInfo.usage = (isDepth ? VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT : VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT) | VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
        imageInfo.samples = VK_SAMPLE_COUNT_1_BIT;
        imageInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
        ...
        if (vmaCreateImage(allocator, &imageInfo, &allocInfo, &m_Image, &m_Allocation, nullptr) != VK_SUCCESS) {
            VECTOR_LOG_ERROR("Failed to create transient RenderGraph image!");
            return;
        }
        ...
        if (vkCreateImageView(device, &viewInfo, nullptr, &m_ImageView) != VK_SUCCESS) {
            VECTOR_LOG_ERROR("Failed to create transient RenderGraph image view!");
            return;
        }
        ...
        if (vkCreateSampler(device, &samplerInfo, nullptr, &m_Sampler) != VK_SUCCESS) {
            VECTOR_LOG_ERROR("Failed to create transient RenderGraph sampler!");
        }
    }
    ...
    VulkanTexture2D::VulkanTexture2D(uint32_t width, uint32_t height, TextureFormat format, Texture2D* alias)
        : m_Width(width), m_Height(height), m_Channels(4) {
        ...
        VulkanTexture2D* vkAlias = dynamic_cast<VulkanTexture2D*>(alias);
        if (!vkAlias || vkAlias->GetAllocation() == VK_NULL_HANDLE) {
            VECTOR_LOG_ERROR("Failed to alias VulkanTexture2D: Alias is invalid.");
            return;
        }

        if (vmaCreateAliasingImage(allocator, vkAlias->GetAllocation(), &imageInfo, &m_Image) != VK_SUCCESS) {
            VECTOR_LOG_ERROR("Failed to create transient aliased RenderGraph image!");
            return;
        }
