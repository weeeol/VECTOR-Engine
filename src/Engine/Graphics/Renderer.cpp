...
namespace VECTOR {
    ...
    void Renderer::SubmitMesh(const Mesh* mesh, const Material* material, const glm::mat4& model, const std::vector<glm::mat4>* boneTransforms) {
        RenderCommand cmd;
        cmd.mesh = mesh;
    ...
    void Renderer::ExecuteRenderPipeline() {
        ...
        BeginShadowPass();
        FlushShadowPass(); // Backends will loop through m_RenderQueue and draw shadow casters

        BeginPrepass();
        FlushPrepass();    // Backends will loop through m_RenderQueue and draw to G-Buffer

        if (IsSSAOEnabled()) {
            // Generate SSAO based on G-Buffer
            // The backend is responsible for intercepting this if necessary or SSAO is already implicitly chained.
        }

        BeginMainPass();
        FlushMainPass();   // Backends will loop through m_RenderQueue and draw to HDR buffer
