...
namespace VECTOR {
    ...
    bool Renderer::Initialize(const std::string& title, int width, int height) {
        ...
        m_PerFrameUBO = std::make_unique<UniformBuffer>(static_cast<uint32_t>(sizeof(PerFrameData)), 0);

        // Bind the UBO block to all 3D shaders that use it
        ...
        m_LightUBO = std::make_unique<UniformBuffer>(static_cast<uint32_t>(sizeof(LightUBOData)), 1);
        ...
        m_DepthMaterial->isUnlit = true; // No lighting needed for depth
    ...
    void Renderer::SetupFBOs() {
        ...
        // 2. Post process FBO — HDR (GL_RGBA16F)
    ...
    void Renderer::RenderBloomPasses() {
        ...
        // First downsample: from HDR scene texture with threshold
        ...
        // Subsequent downsamples: no threshold
    ...
    // =========================================================================
    // NEW: Material-based submit path
    // =========================================================================

    void Renderer::SubmitMesh(const Mesh* mesh, const Material* material, const glm::mat4& model) {
        if (!mesh) return;

        // Frustum Culling
        const AABB& localAABB = mesh->GetAABB();
        
        // Transform AABB to world space (simple approximation using center and extents)
        glm::vec3 globalCenter = glm::vec3(model * glm::vec4(localAABB.center, 1.0f));
        
        // Extract scale from model matrix correctly
        ...
        // Also submit to shadow queue with depth material
        RenderCommand shadowCmd;
        shadowCmd.sortKey = 0; // All same shader, no need to sort
        shadowCmd.mesh = mesh;
    ...
    void Renderer::SubmitPointLight(const glm::vec3& position, float radius, const glm::vec3& color, float intensity) {
    ...
    void Renderer::FlushShadowPass() {
        // Flush the shadow queue — all commands use the depth material
    ...
    void Renderer::BeginMainPass() {
        ...
        // Bind shadow map to texture unit 1 for all main pass draws
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(GL_TEXTURE_2D, m_DepthMapTexture);
    }

    void Renderer::FlushMainPass() {
        // Upload light data to UBO before flushing
        m_LightUBO->SetData(&m_LightData, sizeof(LightUBOData));
        
        // Reset point lights for next frame
        m_LightData.numPointLights = 0;

        // Ensure the UBOs are bound
        m_PerFrameUBO->Bind();
        m_LightUBO->Bind();
        ...
        // Flush sorted render commands
        m_MainQueue.SortAndFlush();
        m_LastFrameDrawCalls += m_MainQueue.GetLastDrawCallCount();
    }

    void Renderer::EndPostProcessPass() {
        // Run bloom passes on the HDR scene texture
        RenderBloomPasses();

        // Final composite: HDR scene + bloom → LDR with tone mapping
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        glViewport(0, 0, m_Width, m_Height); // Restore viewport!
        glDisable(GL_DEPTH_TEST);
